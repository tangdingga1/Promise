...
export default function Resolve(promise, x, promiseFulfilled, promiseRejected) {
...
function resolvedByX(promise, x, promiseFulfilled, promiseRejected) {
  switch(x.status) {
    case 'pending':
      // x 可能是个循环的 promise 链
...
function resolvedByXThen(promise, x, promiseFulfilled, promiseRejected) {
  // 首先取得 x 的then
  let then;
